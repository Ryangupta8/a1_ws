...
namespace a1_controller {
    ...
    A1JointController::A1JointController(){}

    A1JointController::~A1JointController(){
        sub_cmd.shutdown();
    }

    void A1JointController::setCommandCB(const a1_msgs::MotorCmdConstPtr& msg)
    {
    ...
    bool A1JointController::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &n)
    {
        ...
        sub_cmd = n.subscribe("command", 20, &A1JointController::setCommandCB, this);
 
        // Start realtime state publisher
        controller_state_publisher_.reset(
            new realtime_tools::RealtimePublisher<a1_msgs::MotorState>(n, name_space + "/state", 1));        
        return true;
    }

    void A1JointController::setGains(const double &p, const double &i, const double &d, const double &i_max, const double &i_min, const bool &antiwindup)
    {
        pid_controller_.setGains(p,i,d,i_max,i_min,antiwindup);
    }

    void A1JointController::getGains(double &p, double &i, double &d, double &i_max, double &i_min, bool &antiwindup)
    {
        pid_controller_.getGains(p,i,d,i_max,i_min,antiwindup);
    }

    void A1JointController::getGains(double &p, double &i, double &d, double &i_max, double &i_min)
    {
        bool dummy;
    ...
    void A1JointController::starting(const ros::Time& time)
    {
    ...
    void A1JointController::update(const ros::Time& time, const ros::Duration& period)
    {
    ...
    void A1JointController::stopping(){}

    void A1JointController::positionLimits(double &position)
    {
    ...
    void A1JointController::velocityLimits(double &velocity)
    {
        if (joint_urdf->type == urdf::Joint::REVOLUTE || joint_urdf->type == urdf::Joint::PRISMATIC)
            clamp(velocity, -joint_urdf->limits->velocity, joint_urdf->limits->velocity);
    }

    void A1JointController::effortLimits(double &effort)
    {
        if (joint_urdf->type == urdf::Joint::REVOLUTE || joint_urdf->type == urdf::Joint::PRISMATIC)
...
PLUGINLIB_EXPORT_CLASS(a1_controller::A1JointController, controller_interface::ControllerBase);
